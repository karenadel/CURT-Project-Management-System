...
> **Live demo:** _TODO: add deployed link (Vercel / Netlify / GitHub Pages)_
...
> _TODO: complete this section once the app is deployed._
 
- **Platform:** _e.g. Vercel / Netlify / GitHub Pages_
- **Live URL:** _TODO_
- **Build command:** `npm run build`
- **Output directory:** `dist`
- **Notes:** _TODO: e.g. SPA fallback so that React Router routes work on refresh_
> **SPA routing note:** because the app uses client-side routing, the host must redirect all paths to `index.html`.
> - **Netlify:** add `public/_redirects` containing `/* /index.html 200`
> - **Vercel:** add a `vercel.json` with a rewrite of `/(.*)` to `/index.html`
> - **GitHub Pages:** use a `404.html` fallback or a hash router
 
If the app is not deployed, follow [Setup & Running Locally](#setup--running-locally).
